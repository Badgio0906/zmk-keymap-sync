...
5. **調査・修正に着手する前に、必ず `git fetch` を実行し `origin/main` の最新状態を
   確認すること。** ローカル clone が古いまま調査すると、既に解決済み/変更済みの
   内容を前提に誤った調査・修正をしてしまう（実例: ローカルの古い clone で
   調査した結果、リモートで既に行われていた変更に気づかず誤った結論を報告した
   事故があった）
